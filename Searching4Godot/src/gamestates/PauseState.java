...
public class PauseState extends AbstractGameState{
		...
		//Variables
		private String[] options = {"Resume","Restart Level","Save", "Options","Controls","Menu"};
		private int listPosition;
		private LevelManager levelManager;
		private MainGameState mainGameState;
		//Variables
		
		public PauseState(GameStateManager gsm, LevelManager levelManager, MainGameState mainGameState){
			super(gsm);
			listPosition = 0;
		...
		public void keyPressed(int k) {
			...
			if(k == KeyEvent.VK_ENTER){
				if(listPosition == 0)gsm.GameStates.pop();
					...
					try {
						...
						System.out.println(Save.readSave(mainGameState.save));
					} catch (IOException e) {
						e.printStackTrace();
					}
				else if(listPosition == 5)gsm.GameStates.push(new MenuState(gsm));
			}
